...
public class CommandBuilder {
    ...
    private static final TreeMap<String, BaseCommand> commands = new TreeMap<>();
    ...
    public static BaseCommand parseInteractiveCommand(String line, MicroShell microShell) {
        try {
            String[] args = line.split(" ");
            if (!commands.containsKey(args[0])) {
                System.out.println(Phrases.UnregisteredCommand);
                return null;
            }
            BaseCommand prototype = commands.get(args[0]).setParentShell(microShell);
            if (prototype.parseCommand(args)) return (BaseCommand) prototype.clone();
        } catch (Throwable ignored) {
        }
        System.out.println(Phrases.WrongCommandSpelling);
        return null;
    }
    ...
    public static LinkedList<BaseCommand> parseCommandFromScript(MicroShell microShell, String[] lines) {
        try {
            ...
            for (int offset = 0; offset < lines.length; ++offset) {
                ...
                if (cntLines > 0)
                    queue.add((BaseCommand) prototype.clone());
                else {
